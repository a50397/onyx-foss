...
Docker-friendly version: connects to Postgres via DATABASE_URL instead of docker exec.

Usage:
    DATABASE_URL=postgresql://postgres:password@relational_db:5432/postgres \
        python kg_dump_person_docker.py

    # List all persons
    python kg_dump_person_docker.py

    # Dump graph for a person (fuzzy match)
    python kg_dump_person_docker.py kopacik
    python kg_dump_person_docker.py "Patrik Brandýs"
"""
...
def query_rows(sql: str, cols: list[str]) -> list[dict[str, str]]:
    ...
    for raw in raw_rows:
        if len(raw) >= len(cols):
            rows.append({c: str(raw[i]) if raw[i] is not None else "" for i, c in enumerate(cols)})
    return rows
