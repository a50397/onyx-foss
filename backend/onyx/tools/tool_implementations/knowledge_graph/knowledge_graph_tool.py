...
class KnowledgeGraphTool(Tool[KnowledgeGraphToolOverrideKwargs]):
    ...
    def _generate_cypher(self, query: str) -> str | None:
        ...
        system_msg = (
            ...
            "   IMPORTANT — SEARCH TERM SELECTION: For multi-word entity names, use the\n"
            "   MOST DISTINCTIVE word, not a generic one. 'Ministerstvo vnútra' → search\n"
            "   for 'vnutra' (unique), NOT 'ministerstvo' (matches every ministry).\n"
            "   'Slovenská sporiteľňa' → 'sporitel' (unique), NOT 'slovenska' (too generic).\n"
            "   When in doubt, use multiple CONTAINS with AND to be specific.\n"
            ...
            "13. PROJECT CUSTOMER SEARCH: When searching for projects related to a\n"
